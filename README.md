...
The goal of this report is exploring  the various steps and procedures  in order to create a proper spreadsheet with its own set of tools such as a **searching engine** and so on.
we're going specific in the coming sections below.
...
```
> After creating the interface and templates, we're adding the tools we already mentionned .
...
```
>  we can find any cell by typing the correct regular expression otherwise it wont accept it.
...
```
>Now we get the ability to search for any data inside our spreadsheet .
# Conclusion 

I managed to create a spreadsheet from scratch which is cool , using both **UI Widgets** and **Brute Coding** , it's been an entertaining experience from which i've learned a lot of useful stuff .
